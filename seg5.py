...
save_background_img = False  # 是否同步生成白背景图片
...
if os.path.exists(skip_dir) and os.path.isdir(skip_dir):
    skip_files = [f for f in os.listdir(skip_dir) if
                  f.lower().endswith(('.png', '.jpg', '.jpeg', '.JPG', '.JPEG', '.PNG'))]
...
def apply_mask(image, mask, alpha_channel=True):
    ...
    images = [image]
    if save_background_img:
        image_white = np.where(mask[..., None] == 1, image, 255)
        images.append(image_white)

    for i, im in enumerate(images):
        if alpha_channel and im.shape[-1] == 3:
            alpha = np.zeros_like(im[..., 0])
            alpha[mask == 1] = 255
            images[i] = cv2.merge((im[..., 0], im[..., 1], im[..., 2], alpha))
    return images
...
def get_next_filename(base_path, filename):
    ...
    for i in range(1, 101):
        ...
        names = [new_name]
        if save_background_img:
            new_name_white = f"{name}_{i}w{ext}"
            names.append(new_name_white)
        if not os.path.exists(os.path.join(base_path, names[0])) and (len(names) == 1 or not os.path.exists(
                os.path.join(base_path, names[1]))):
            return names
    return None
...
def save_masked_image(image, mask, output_dir, filename, crop_mode_):
    if crop_mode_:
        ...
        masked_images = apply_mask(cropped_image, cropped_mask)
    else:
        masked_images = apply_mask(image, mask)

    filename = filename[:filename.rfind('.')] + '.png'
    new_filenames = get_next_filename(output_dir, filename)

    if new_filenames and masked_images:
        for new_filename, masked_image in zip(new_filenames, masked_images):
...
def show_infos(info, show, image_to_show):
    if not show:
        return

    for i, inf in enumerate(info):
        cv2.putText(image_to_show, inf, (10, 30 + 40 * i), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2, cv2.LINE_AA)
...
while True:
    ...
    image_orign = cv2.imread(os.path.join(input_dir, filename), cv2.IMREAD_UNCHANGED)
    ...
    while True:
        ...
        display_info = f'{filename} | Press s to save | Press w to predict'
        display_info1 = f'| Press a to previous image | Press d to next image | Press space to clear'
        display_info2 = f'| Press q to remove last point | Press +,- to zoom in or out | Press e to hide suggestion info'
        infos = [display_info, display_info1, display_info2]
        show_infos(infos, show_info, image_display)
        ...
        if key == ord(" "):
            ...
            if len(input_point) > 0 and len(input_label) > 0:
                ...
                while (1):
                    ...
                    mask_info = f'Total: {num_masks} | Current: {mask_idx} | Score: {scores[mask_idx]:.2f} | Press w ' \
                                f'to confirm'
                    mask_info1 = f'| Press d to next mask | Press a to previous mask | Press q to remove last point  '
                    mask_info2 = f'| Press s to save | Press +,- to zoom in or out | Press e to hide suggestion info'
                    infos = [mask_info, mask_info1, mask_info2]
                    show_infos(infos, show_info, selected_image)

                    selected_image = cv2.resize(selected_image, None, fx=zoom_rate, fy=zoom_rate,
